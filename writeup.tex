...
\usepackage{xcolor} \pagecolor[RGB]{38,50,56} \color[RGB]{225,246,246}
...
% We recreate Lemma 14.1 with a variable learning rate
% \(\eta_t = \frac{B}{\rho \sqrt t}\)

% \begin{align*}
% \langle w^{(t)} - w^*, v_t \rangle &= \frac{1}{\eta_t} \langle w^{(t)} - w^*, \eta_tv_t \rangle \\
% &= \frac{1}{2\eta_t} \left(-||w^{(t+1)} - w^*||^2 + ||w^{(t)} - w^*||  \right) + \frac{\eta_2}{2} ||v_t||^2

% \end{align*}

% Summing over all iterations \(T\)
